 ...
 *
 *   gcc principal.c -o test -DPARALLEL=N -fopenmp -Wall
 *   Ejemplo:
 *   gcc principal1.c -o test -fopenmp -Wall -DPARALLEL=100
 ********************************************************/
...
grey_image_type *transform(grey_image_type *greyImage) {
    ...
    int *H, *C;
    H = calloc(NB_PIXELS, sizeof(int));  // inicializar en cero
...
#ifdef PARALLEL
        // Esta instrucción debe ser atómica; de otra manera
        // se pueden hacer múltiples accesos y modificaciones al mismo tiempo
#pragma omp critical
        {
#endif
            H[pixel]++;
...
#ifdef PARALLEL
    // Podemos paralelizar este lazo porque cada instrucción
    // es independiente en cada iteración de i
...
int main(int argc, char *argv[]) {
    saveGreyImage(
        "src/resize/image0_contraste.pgm",
        transform(colorImageToGrey(loadColorImage("src/image0.ppm")))
    );

    saveGreyImage(
        "src/resize/image2_contraste.pgm",
        transform(colorImageToGrey(loadColorImage("src/image2.ppm")))
    );
