/*******************************************************
 * Download images files from:
 *   https://u.pcloud.link/publink/show?code=3O7ctalK
 *     - image0.ppm
 *     - image2.ppm
 *
...
/**********************************************************************
 * Crea la representación gris en memoria de la imagen color
 **********************************************************************/
...
/*********************************************************************
 * Guardamos la imagen gris en un nuevo archivo PGM
 *********************************************************************/
...
#pragma omp parallel for private(r, g, b) num_threads(PARALLEL)
    ...
    for (int i = 0; i < width * height; i++) {
        ...
        // Convertir los píxeles RGB a gris utilizando esta propiedad matemática
...
int main(int argc, char *argv[]) {
    saveGreyImage(
        "src/resize/image0_grey.pgm",
        colorImageToGrey(loadColorImage("src/image0.ppm"))
    );

    saveGreyImage(
        "src/resize/image2_grey.pgm",
        colorImageToGrey(loadColorImage("src/image2.ppm"))
    );
