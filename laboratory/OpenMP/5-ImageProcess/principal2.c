...
#pragma omp parallel for num_threads(PARALLEL)
    ...
    for (int i = 0; i < width * height; i++) {
        if (i < width || i % width == 0 || (i + 1) % width == 0 || (height - 1) * width <= i) {
            ...
            colorImage->pixels[i] =
                greyPixelToColor(greyImage->pixels, i, width, height);
        }
    }
...
int main(int argc, char *argv[]) {
    saveGreyImage(
        "src/resize/image0_color.pgm",
        greyImageToColor(colorImageToGrey(loadColorImage("src/image0.ppm")))
    );

    saveGreyImage(
        "src/resize/image2_color.pgm",
        greyImageToColor(colorImageToGrey(loadColorImage("src/image2.ppm")))
    );
